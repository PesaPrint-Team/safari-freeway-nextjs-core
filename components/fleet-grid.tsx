    ...
    <section id="fleet" className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
      ...
      <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
        {FLEET.map((item, index) => {
          ...
          return (
            <Card key={item.name} className="shadow-2xl shadow-black/20 transition duration-300 hover:-translate-y-1 hover:bg-white/[0.07] hover:shadow-black/30">
              <CardContent>
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="text-xs uppercase tracking-[0.25em] text-white/45">{item.type}</div>
